...
// Port of glsl/cell_text.v.glsl. Changes:
//  - SSBO moved to set 0, binding 1 (see common.glsl for the layout).
//  - gl_VertexID -> gl_VertexIndex.
//  - Explicit location on the output interface block (required by
//    Vulkan GLSL).
...
layout(location = 0) out CellTextVertexOut {
    flat uint atlas;
    flat vec4 color;
    flat vec4 bg_color;
    vec2 tex_coord;
...
layout(set = 0, binding = 1, std430) readonly buffer bg_cells {
    uint bg_colors[];
};
...
void main() {
    ...
    // (See the original shader for the glyph metrics diagram.)
    ...
    // The fragment stage reads the atlas with texelFetch (the GL backend
    // uses a rectangle texture; Vulkan has none).
